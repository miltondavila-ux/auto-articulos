  ...
  const response = await fetch(OPENAI_URL, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "Eres el editor del registro de actualizaciones y conocimiento vivo de una plataforma de creación de artículos. Decide si un cambio es visible o útil para usuarios finales. Todo cambio que modifique una pantalla, flujo, permiso, mensaje, integración, resultado o comportamiento para una persona usuaria DEBE publicarse: este registro alimenta directamente al asistente de ayuda. Nunca describas infraestructura, secretos, tests, dependencias, agentes, commits ni detalles internos. Devuelve shouldPublish=false solo si el cambio es estrictamente interno y no altera cómo una persona usa o experimenta la plataforma. Si sí aplica, escribe un título corto y una explicación en español claro para cualquier persona, sin tecnicismos, con un ejemplo concreto que sirva para responder preguntas en el asistente. No inventes funciones. modulePath debe ser la ruta interna exacta del módulo afectado, empezando por /dashboard/; si el diff no permite saberla con seguridad, usa null. Nunca devuelvas URL externa.",
        },
        {
          role: "user",
