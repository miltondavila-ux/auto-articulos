...
### Claude — segunda tanda de Bing (cuenta de Lorena Álvarez, 13/8/2026)

- **Estado:** `IMPLEMENTADO LOCALMENTE — SIN COMMIT NI DESPLIEGUE` (13/8/2026).
- **Reserva de archivos:**
...
- **Encontrado sin arreglar (fuera del alcance de este pedido, decisión de Milton):** (1) `apps/web/src/app/api/bing/master-index/route.ts` envía TODOS los artículos publicados cada vez, incluidos los ya enviados, de 10 en 10 en paralelo, sin consultar `GetUrlSubmissionQuota`. La documentación de Microsoft dice explícitamente que hay que consultar la cuota antes porque el cupo diario es limitado, y el comentario del código asume 10.000/día, que aplica a sitios grandes, no a los de estos clientes. (2) `BingWebmasterSection.tsx` lee `value.yaIndexados` de la respuesta de master-index, pero esa ruta nunca devuelve ese campo — quedó de una versión que sí salteaba los ya enviados. (3) El comentario de `apps/worker/src/bingIndexing.ts:14` documenta que el `InvalidToken` aparecía de forma INTERMITENTE dentro de un mismo lote, lo que apunta a cuota/throttling y no a un token inválido.
