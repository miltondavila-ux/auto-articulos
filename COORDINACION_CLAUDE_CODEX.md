...
- Actualización de cierre: la migración idempotente de progreso social se ejecutó en Supabase SQL Editor con resultado `Success. No rows returned`. El lote aislado se publicó en `main` como `a6b3ea9`; Vercel creó `dpl_nqAXPCqE7RwwoWp8S77b4L8kkXCJ` en estado `Ready` (producción). El workflow del worker `32076653980` está en cola porque existe otro worker anterior en ejecución; no se canceló para evitar interrumpir publicaciones ajenas.
...
### 2026-08-23 (cont. 11) — Claude: espaciado del botón "Comienza aquí" + gráfico de Tremor sin línea visible

- Milton reportó, con captura, dos problemas en Inicio:
  1. El botón "Comienza aquí" se veía pegado al borde inferior de la tarjeta de introducción, como roto/superpuesto.
  2. El gráfico "Tu ritmo — últimos 14 días" no dibujaba ninguna línea, aunque el texto debajo sí mostraba datos reales ("132 de 1260 artículos").
- **Arreglo 1:** `apps/web/src/app/dashboard/page.tsx` — el `<div>` que envuelve el botón solo tenía `marginBottom: 16`, sin `marginTop`. Se agregó `marginTop: 20`.
- **Arreglo 2 (causa raíz real, no cosmética):** los componentes de Tremor (`LineChart`, `Badge`, `ProgressBar`, `Callout`) arman sus clases de color con template literals en tiempo de ejecución — `getColorClassNames()` en `@tremor/react/dist/lib/utils.js` devuelve cosas como `` `stroke-${color}-${shade}` ``. Tailwind nunca ve el nombre de clase completo en el código fuente y las purga del build de producción. El `safelist` de `apps/web/tailwind.config.js` solo cubría la paleta personalizada `tremor-*` (fondo/borde de tarjetas), nunca la paleta estándar (`neutral`, `gray`, `blue`, `emerald`...) que se usa para las SERIES de datos y los badges. **Verificado empíricamente** con `npx tailwindcss -i globals.css -o out.css`: antes del fix, `stroke-neutral-500` no existe en el CSS compilado (0 ocurrencias); después, sí. Se agregó una segunda regla de `safelist` con la paleta completa de Tailwind (`bg|text|border|ring|fill|stroke` × 21 colores × 11 tonos). El CSS compilado creció de ~29k a ~49k líneas — costo aceptado, es exactamente el safelist que Tremor recomienda en su propia documentación de instalación.
  - Commit `f410bdb`.
- **Cambio de infraestructura, mismo lote:** se reemplazó el workflow de un solo uso `backfill-product-updates-20260823.yml` por uno reutilizable, `.github/workflows/run-product-update-script.yml` (input `script_path`), para no llenar `.github/workflows/` con un archivo nuevo cada vez que se agrega una entrada al changelog — cumpliendo el compromiso adquirido en la entrada anterior de este mismo día. El input pasa por variable de entorno (`SCRIPT_PATH`), no por interpolación directa en `run:`, para evitar inyección de comandos vía el input de `workflow_dispatch`.
