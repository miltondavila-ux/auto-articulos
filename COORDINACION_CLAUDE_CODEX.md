...
- **Pendiente real que queda:** confirmar si este fix resuelve el "bucle" por completo (probable, ya que la causa más probable era el doble clic) o si el `ERROR!!! InvalidToken` de Master Indexación (hipótesis: scope OAuth insuficiente para `SubmitUrl`) sigue apareciendo incluso con una reconexión limpia de un solo clic — eso sí seguiría siendo un problema aparte, sin resolver.
