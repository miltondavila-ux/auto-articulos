...
export async function GET() {
  ...
  const envClientId = process.env.TWITTER_CLIENT_ID;
  const envClientSecret = process.env.TWITTER_CLIENT_SECRET;
...
export async function POST(request: NextRequest) {
  ...
  if (typeof clientId !== "string" || typeof clientSecret !== "string" || !clientId || !clientSecret) {
    return NextResponse.json(
      { error: "Debes ingresar tanto el Client ID como el Client Secret de X (Twitter)." },
      { status: 400 }
    );
  }
  ...
  await Promise.all([
    prisma.systemSetting.upsert({
      ...
      create: { key: "twitter_client_id", encryptedValue: encryptSecret(clientId.trim()) },
      update: { encryptedValue: encryptSecret(clientId.trim()) },
    }),
    prisma.systemSetting.upsert({
      where: { key: "twitter_client_secret" },
      create: { key: "twitter_client_secret", encryptedValue: encryptSecret(clientSecret.trim()) },
