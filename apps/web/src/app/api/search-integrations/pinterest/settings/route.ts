...
export async function GET() {
  ...
  if (user.role !== "admin") {
    return NextResponse.json({ configured, isAdmin: false }, { headers: NO_CACHE });
  }
...
export async function POST(request: NextRequest) {
  ...
  if (typeof body.clientId !== "string" || typeof body.clientSecret !== "string" || !body.clientId.trim() || !body.clientSecret.trim()) {
    return NextResponse.json({ error: "Debes ingresar el Client ID y el Client Secret de Pinterest." }, { status: 400 });
  }
  await Promise.all([
    prisma.systemSetting.upsert({
      where: { key: "pinterest_client_id" },
      create: { key: "pinterest_client_id", encryptedValue: encryptSecret(body.clientId.trim()) },
      update: { encryptedValue: encryptSecret(body.clientId.trim()) },
    }),
    prisma.systemSetting.upsert({
      where: { key: "pinterest_client_secret" },
      create: { key: "pinterest_client_secret", encryptedValue: encryptSecret(body.clientSecret.trim()) },
      update: { encryptedValue: encryptSecret(body.clientSecret.trim()) },
    }),
  ]);
