...
export const MCP_SCOPES = ["oportunidades:leer", "oportunidades:publicar"] as const;
...
export function oauthClient() {
  return {
    clientId: requiredEnv("OAUTH_ALEXA_CLIENT_ID"),
    clientSecret: requiredEnv("OAUTH_ALEXA_CLIENT_SECRET"),
    redirectUris: requiredEnv("OAUTH_ALEXA_REDIRECT_URIS").split(",").map((uri) => uri.trim()),
  };
}
...
export function parseScopes(scope: string | null) {
  ...
  if (scopes.length === 0 || scopes.some((item) => !MCP_SCOPES.includes(item as (typeof MCP_SCOPES)[number]))) {
...
export function validateAuthorizeRequest(params: URLSearchParams, origin: string) {
  ...
  if (clientId !== client.clientId) throw new OAuthError("unauthorized_client", "Cliente OAuth no reconocido.");
  ...
  return { clientId, redirectUri, resource, codeChallenge, scopes: parseScopes(params.get("scope")), state: params.get("state") };
}
...
async function issueTokens(data: { userId: string; clientId: string; scopes: string; resource: string }) {
  const accessToken = await createMcpAccessToken(data.userId);
