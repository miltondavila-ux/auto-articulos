...
REGLA ANTI-INVENCIÓN — LA MÁS IMPORTANTE DE TODAS, LÉELA CON CUIDADO:
Ya pasó antes que un análisis inventó una lista de títulos mencionando un estado de EE. UU. distinto en cada uno (Georgia, Texas, California, Nevada, Illinois...) sin ninguna evidencia real de que este usuario tenga tráfico o contenido relacionado con esos lugares — eso es fabricar datos y está terminantemente prohibido. Reglas estrictas:
- NUNCA menciones una ciudad, condado, estado, país o nacionalidad específica en un título a menos que ese nombre EXACTO aparezca literalmente en: (a) el texto de una consulta real, (b) una página real, o (c) un título que este usuario YA publicó (ver TÍTULOS YA EXISTENTES).
...
- Si ninguna consulta, página o título existente menciona una ubicación específica para una categoría, esa categoría NO lleva ubicación — construye el long tail alrededor del perfil de cliente y el producto/servicio únicamente. Es preferible un título sin ubicación a uno con una ubicación inventada.
- El campo "rationale" de cada título debe citar o parafrasear muy de cerca la evidencia real (la consulta, página o título existente concreto) que respalda cualquier ubicación o perfil de cliente que uses. Si no puedes señalar esa evidencia concreta, no la incluyas.
...
- Si genuinamente no hay evidencia suficiente para armar ni un solo grupo de 9 títulos únicos, no duplicados y sin ubicaciones inventadas, responde exactamente {"opportunities":[]} — no inventes títulos débiles ni geografía falsa solo para llenar el formato.
