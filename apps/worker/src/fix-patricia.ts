...
export async function runPatriciaFix(
  ...
  try {
    ...
    // Recopilar hasta MAX_REPAIRS_PER_RUN artículos no procesados navegando página por página de forma natural
    while (hasNextPage && idsToProcess.length < MAX_REPAIRS_PER_RUN) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (idsToProcess.length >= MAX_REPAIRS_PER_RUN) {
          break;
        }
      }
      ...
      // Avanzar a la siguiente página usando el botón Siguiente nativo
    ...
    if (idsToProcess.length > 0) {
      ...
      await onStep(`Iniciando lote: Se revisarán ${idsToProcess.length} artículos (desde ID [${startId}] hasta ID [${endId}])...`);
          ...
          try {
            ...
            if (
              ...
              if (await saveBtn.isVisible()) {
                ...
                successCount++;
                await onStep(`✓ ¡Reparado con éxito! (${successCount} de ${idsToProcess.length}): ${article.title} — Enlace: ${article.publicUrl || article.editUrl}`);
                success = true;
              } else {
      ...
      await onStep("No se encontraron más artículos pendientes de revisión (todos procesados o cuenta al día).");
    }

    return { repaired: successCount, alreadyCorrect: skippedCount, failed: errorCount, hasNextPage: idsToProcess.length >= MAX_REPAIRS_PER_RUN };
