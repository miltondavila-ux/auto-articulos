...
// Un artículo puede tardar varios minutos por IA, imagen y validaciones
// externas, pero nunca debe dejar un lane ocupado indefinidamente si el sitio
// deja de responder. Al vencer este límite se cierra Chromium y queue.ts
// registra el artículo como fallo recuperable.
  ...
  try {
    ...
    const timeout = new Promise<PublishResult>((_, reject) => {
      watchdog = setTimeout(() => {
        ...
        reject(
          new Error(
            "El artículo superó el tiempo máximo de procesamiento y se devuelve a Oportunidades para reintentarlo.",
          ),
        );
